...
def manage_users_data():
    ...
    user_rows = User.get_users_with_traffic()
    html = render_template(
        'manageusers_list.html',
        users=[row[0] for row in user_rows],
        rejected_users=User.get_rejected(),
        all_last_month_traffic={row[0].email: int(row[1]) for row in user_rows},
        all_month_traffic={row[0].email: int(row[2]) for row in user_rows},
    )
